...
def get_database(model_path):
    ...
    if ('clea' in model_path):
        pretrain_database = 'USPTO_SMILES_clean'
    if ("chem" in model_path) or ("Chem" in model_path):
        pretrain_database = 'ChEMBL'
    if ("CEP" in model_path):
        pretrain_database = 'CEPDB'
    if ("rxn" in model_path):
        pretrain_database = 'rxnfp'
    if ("deepchem" in model_path.lower()):
        pretrain_database = 'DeepChem-77M'
    ...
    if ('Absorption' in model_path) or ("MAW" in model_path) or ('abs' in model_path):
        fine_tuning_database = 'EOO_MAW'
        df_test_path = 'data/fine_turning_data/EOO/EOO_abs_clean_test.csv'
    if ('Emission' in model_path) or ("MEW" in model_path) or ('emi' in model_path):
        fine_tuning_database = 'OLED_MEW'
