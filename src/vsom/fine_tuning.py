...
class TrainingPipeline:
    ...
    def load_and_preprocess_data(self):
        ...
        eval_df, test_df = train_test_split(
            eval_df, test_size=0.5, random_state=self.seed
        )

        # Save split datasets
        train_df.to_csv(
            self.data_path.replace(".csv", "_train.csv"), encoding="utf-8", index=False
        )
        eval_df.to_csv(
            self.data_path.replace(".csv", "_val.csv"), encoding="utf-8", index=False
        )
        test_df.to_csv(
            self.data_path.replace(".csv", "_test.csv"), encoding="utf-8", index=False
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--data_name", type=str, required=True, help="Name of the dataset"
    )
    parser.add_argument(
        "--data_path", type=str, required=True, help="Path to the dataset CSV file"
    )
    parser.add_argument(
        "--labels", type=str, required=True, help="Column name for labels"
    )
    parser.add_argument(
        "--pre_model", type=str, required=True, help="Pretrained model name"
    )
...
if __name__ == "__main__":
    ...
    pipeline = TrainingPipeline(
        args.data_name, args.data_path, args.labels, args.pre_model
    )
    pipeline.train()
