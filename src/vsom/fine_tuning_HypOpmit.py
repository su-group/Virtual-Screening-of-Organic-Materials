...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Fine tuning  Hyperparams Optimization")
    parser.add_argument("--data_path", type=str, default='data/solar/solar.csv', help="Path to the data file.")
    parser.add_argument("--pre_model_name", type=str, default="USPTO_SMILES_clean",
                        help="Name of the pre-trained model.")
    parser.add_argument("--data_name", type=str, default='solar', help="Name of the dataset.")
    parser.add_argument("--labels", type=str, default='KS_gap', help="Label column name.")
...
def train_model(train_df: pd.DataFrame, eval_df: pd.DataFrame):
    ...
    pretrained_bert.train_model(train_df, output_dir=f'out/{args.pre_model_name}-{args.data_name}-{args.labels}_hy',
...
def main():
    ...
    # Start sweep
    sweep_id = wandb.sweep(sweep_config, project=f'out/{args.pre_model_name}-{args.data_name}-{args.labels}_hy')
