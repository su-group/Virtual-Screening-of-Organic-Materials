...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Pretraining Pipeline for Smiles Language Modeling"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--data_name", type=str, default="USPTO", help="Name of the dataset"
    )
    parser.add_argument(
        "--split_ratio", type=float, default=0.1, help="Ratio for train-test split"
    )
    parser.add_argument(
...
class PretrainingPipeline:
    def __init__(
        self, data_path, output_dir, data_name="USPTO", split_ratio=0.1, seed=42
    ):
    ...
    def prepare_data(self):
        ...
        train, test = train_test_split(
            df, test_size=self.split_ratio, random_state=self.seed
        )
        train.to_csv(
            f"{self.data_path}/{self.data_name}_train.csv",
            encoding="utf-8",
            index=False,
        )
        test.to_csv(
            f"{self.data_path}/{self.data_name}_val.csv", encoding="utf-8", index=False
        )
    ...
    def train_model(self):
        ...
        model.train_model(
            train_file=f"{self.data_path}/{self.data_name}_train.csv",
            eval_file=f"{self.data_path}/{self.data_name}_val.csv",
        )
